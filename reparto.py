...
# --- 1. FUNCIÓN DE LIMPIEZA DE DEUDA FIJA ---
def limpiar_deuda(valor):
...
if st.session_state.reparto is None:
    ...
    if st.button("Iniciar Reparto"):
        ...
        clientes_filtrados = [r for r in st.session_state.data_control if st.session_state.mapping_zonas.get(str(r.get("Cliente", "")).strip()) == reparto_elegido]
        st.session_state.lista_filtrada = sorted(clientes_filtrados, key=lambda x: int(x.get("salida", 9999)))
        st.session_state.idx_cliente = 0
        st.rerun()
else:
    lista = st.session_state.lista_filtrada
    if not lista:
        st.warning("No se encontraron clientes en esa zona.")
    ...
    # --- LECTURA DEUDA ANTERIOR (COLUMNA C) ---
    # Convertimos las llaves a minúsculas para encontrar "deuda anterior" aunque tenga espacios o mayúsculas
    deuda_raw = 0.0
    for key, value in cliente.items():
        if "deuda" in key.lower():
            deuda_raw = value
            break
            
    deuda_float = limpiar_deuda(deuda_raw)
    
    if st.button(f"⬅️ Cambiar Zona ({st.session_state.reparto})"):
    ...
    if st.button("🚚 CARGAR PAGO", type="primary", use_container_width=True):
